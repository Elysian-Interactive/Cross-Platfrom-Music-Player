...
class Test(QWidget):
    ...
    def setupUI(self):
        ...
        # TEMPORARY TEST
        # -> manually adding the widget in the scrollable area
        # -> Adding a separtor between songs
        self.songs_box = QVBoxLayout()
        for i in range (0,20):
            widget = VisibleSong(str(i))
            widget.play_song_but.clicked.connect(lambda _, w=widget: self.playMe(w.title)) # Connecting to a temporary function
            # By using w=widget as a default argument, each lambda function will capture the value of widget at the time it was created.
            self.songs_box.addWidget(widget)  
            # Creating the Separators
            # Must always create a new instance everytime
            separator = QFrame(self)
            separator.setFrameShape(QFrame.HLine)  # Horizontal line separator
            separator.setFrameShadow(QFrame.Sunken)
            self.songs_box.addWidget(separator)
        self.songs_box.addStretch()
    ...
    def playMe(self,value):
        print("Play the song with value",value)
